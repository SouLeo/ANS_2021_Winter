...
def open_annotations(wpath: WindowsPath, name: str):
    ...
    if name == 'arrash':
        ...
        beth_file_names = []
        for i in x:
            beth_file_string = 'umrf_graph_' + str(json_cropped_int[i]) + '_Arrash.umrfg.json'
            beth_file_names.append(beth_file_string)

        json_dicts = []
        for json_file in beth_file_names:
            with open('c:/Users/321785/PycharmProjects/URA_anno_eval_11_16/Arrash/' + json_file, 'r') as f:
                json_string = f.read()
    ...
    if name == 'ian':
        json_list = [pos_json for pos_json in os.listdir(wpath) if pos_json.endswith('.json')]
        json_list_cropped = [e[12:] for e in json_list]
        ...
        sid_file_names = []
        for i in x:
            sid_file_string = 'urmf_iak284_' + str(json_cropped_int[i]) + '.umrfg.json'
            sid_file_names.append(sid_file_string)

        json_dicts = []
        for json_file in sid_file_names:
            with open('c:/Users/321785/PycharmProjects/URA_anno_eval_11_16/Ian/' + json_file, 'r') as f:
                json_string = f.read()
                json_dict = json.loads(json_string)
                json_dicts.append(json_dict)
    return json_dicts
...
if __name__ == '__main__':
    sentences = open_ann_file('c:/Users/321785/PycharmProjects/URA_anno_eval_11_16/group_3_annotation_list.txt')

    c_json_anns = open_annotations(WindowsPath('c:/Users/321785/PycharmProjects/URA_anno_eval_11_16/Arrash'), 'arrash')
    c_filtered_sentences = compare_against_ann_file(sentences, c_json_anns)

    m_json_anns = open_annotations(WindowsPath('c:/Users/321785/PycharmProjects/URA_anno_eval_11_16/Ian'), 'ian')
